...
(function(global) {
  ...
  function noteNameToMidi(name, octave) { const b=NOTE_TO_MIDI[name]; return b===undefined?null:b+((octave+1)*12); }

  let isRecording=false,isPaused=false,startTime=null,pauseOffset=0,pauseStart=null;
  let events=[],takes=[],timerInterval=null,mediaRecorder=null,audioChunks=[];
  ...
  function noteOnByName(name,octave,velocity){const m=noteNameToMidi(name,octave||4);if(m!==null)noteOn(m,velocity);}
  function noteOffByName(name,octave){const m=noteNameToMidi(name,octave||4);if(m!==null)noteOff(m);}
  ...
  function buildMidiFile(evts,bpm){
    ...
    let prev=0;
    evts.forEach(e=>{const tick=Math.round((e.time/1000)*(tpb*bpm/60));const d=Math.max(0,tick-prev);prev=tick;td.push(...varLen(d),e.type,e.note&0x7F,e.velocity&0x7F);});
    td.push(...varLen(0),0xFF,0x2F,0x00);
    return new Uint8Array([0x4D,0x54,0x68,0x64,...int32(6),...int16(0),...int16(1),...int16(tpb),0x4D,0x54,0x72,0x6B,...int32(td.length),...td]);
  }
  ...
  async function startAudio(){
    try{
      ...
      return true;
    }catch(e){console.warn('KCMRecorder: Audio unavailable',e.message);return false;}
  }
  ...
  function setBtn(id,enabled){const e=el(id);if(!e)return;e.style.opacity=enabled?'1':'0.35';e.style.pointerEvents=enabled?'auto':'none';}
  function formatTime(ms){const t=Math.floor(ms/100),tenths=t%10,secs=Math.floor(t/10)%60,mins=Math.floor(t/600);return String(mins).padStart(2,'0')+':'+String(secs).padStart(2,'0')+'.'+tenths;}
...
<div class="kr-row2">
  ...
  async function _toggleRecord(){if(!isRecording){await _startRecord();}else if(!isPaused){_pause();}else{_resume();}}

  async function _startRecord(){
    events=[];startTime=performance.now();pauseOffset=0;isRecording=true;isPaused=false;
    await startAudio();
  ...
  function _rewind(){
    ...
    if(!take.audioBlob){alert('No audio to rewind.');return;}
